...
export function getGeminiModelCandidates(env = process.env) {
...
export async function callGeminiJson({ instructions, input, imageDataUrl, schema, userId, env = process.env }) {
  ...
  for (const model of getGeminiModelCandidates(env)) {
    try {
      const response = await client.models.generateContent({
        ...
        config: {
          ...
          maxOutputTokens: Number(env.AI_MAX_OUTPUT_TOKENS) || 3000,
        },
      });
  ...
  throw new Error(errors.join(' | ') || 'Gemini não respondeu');
}
